"""
ツイートを検索し、DBにURLを保存する
"""

import sys
from pathlib import Path
PARENT=str(Path(__file__).parent)
ROOT=str(Path(__file__).parent.parent)
sys.path.append(ROOT)

import numpy as np
from math import floor
import psycopg2
from datetime import datetime
from copy import deepcopy
import time

from src.auto_twitter import AutoTwitter
from src.filters import bot_filter
from src.envs import *


def main():

    num_tweet_max=SEARCH_AND_GET_TWEET_CFG["NUM_TWEET_MAX"]
    key_words=SEARCH_AND_GET_TWEET_CFG["KEY_WORDS"]
    tab_names=SEARCH_AND_GET_TWEET_CFG["TAB_NAME"]

    now_datetime=datetime.now()

    url="https://twitter.com"
    auto_twitter=AutoTwitter()
    auto_twitter.access_url(url=url)

    #ツイートの検索
    tweet_list=[]
    num_per_query=floor(num_tweet_max/len(key_words)/len(tab_names)) #1検索につき取るtweet数
    print("num_per_query",num_per_query)
    for tab_name in tab_names:
        for keyword in key_words:

            current_height=0

            auto_twitter.search_tweet(keyword=keyword) #検索
            auto_twitter.select_tab(tab_name=tab_name) #タブ選択

            tweet_key_word=[]

            t=time.time()
            while len(tweet_key_word)<num_per_query and (time.time()-t)<30: #30秒検索してたら次行く
                result=auto_twitter.get_tweet(filters=[bot_filter])
                for href,account in result:

                    if len(tweet_list)>0:
                        # print("--href_list")
                        # print(np.array(tweet_list)[:,0])
                        # print("--")
                        if not account in np.array(tweet_list)[:,1]:
                            tweet_key_word+=[[href,account]]
                    else:
                        tweet_key_word+=result

                current_height=auto_twitter.scroll_page(current_height=current_height)

            # print(np.array(tweet_key_word),np.array(tweet_key_word).shape)

            if len(tweet_key_word)>num_per_query:
                tweet_key_word=deepcopy(tweet_key_word[:num_per_query])
            tweet_list+=tweet_key_word
    #
    print("Get following tweet:")
    print(np.array(tweet_list).reshape(-1,2))
    print("---")

    ##データベースへの接続
    conn=psycopg2.connect(
        host=HOST,user=USER,password=PASSWORD,database=DATABASE
    )

    with conn:
        with conn.cursor() as cursor:

            #既に持ってるツイートを取得
            query="SELECT url FROM tweet;"
            cursor.execute(query)
            tweet_urls_db=[tweet_url[0].replace(" ","") for tweet_url in cursor.fetchall()]
            #

            #最大idの取得.自動で振り分けされない.なんでや
            query="SELECT MAX(id) FROM tweet;"
            cursor.execute(query)
            result=cursor.fetchone()
            id_last=int(result[0]) if not result[0] is None else 0
            #

            #新規のツイートをDBに追加
            tweet_db=[]
            idx=1
            for tweet_url,_ in tweet_list:
                if not tweet_url in tweet_urls_db:
                    tweet_db+=[
                        [id_last+idx,tweet_url,now_datetime]
                    ]
                    idx+=1
            print("Register following tweets:")
            print(np.array(tweet_db))
            query=f"INSERT INTO tweet (id,account_id ,url, get_time) VALUES(%s,NULL,%s,%s);"
            cursor.executemany(query,tweet_db)
            #

        conn.commit()


if __name__=="__main__":
    main()

